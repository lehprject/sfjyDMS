...
namespace Dal
{
    public class dr_addvisit_DA : DataAccessBase
    {
        #region  出诊相关 dr_addvisit
             ...
             orderbyEnum? orderby, string orderbyCol, int pageIndex, int pageSize, out string error)
        {
            ...
            try
            {
                ...
                string selectSql = string.Format("select * from dr_pre_addvisit pre left join dr_addvisit on pre.visit_id=dr_addvisit.pkid WHERE TRUE ");
                ...
                var resultList = db.Database.SqlQuery<dr_pre_addvisit>(selectSql, paraList.ToArray()).ToList();
                return resultList;
                #endregion
            }
            catch (Exception ex)
        ...
        {
            return db.dr_pre_addvisit.Where(t => t.addvisit_id == visitID).ToList();
        }
