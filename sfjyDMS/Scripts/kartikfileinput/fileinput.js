...
(function ($) {
    ...
    FileInput.prototype = {
            ...
            self.$preview.find('.kv-file-remove').each(function () {
                ...
                if (vUrl === '#') {
                    $el.off('click').on('click', function () {
                        ...
                        var n = previewCache.count(self.id);
                        var cap = n > 0 ? self.getMsgSelected(n) : '';

                        self.raise('filedeleted', [vKey, null, extraData]);

                        var filestack = self.getFileStack(true)
                        var chk = previewCache.count(self.id);
                        var len = filestack.length
                        cap = self.getMsgSelected(chk + len);
                        self.setCaption(cap);
            ...
            fnSuccess = function (data, textStatus, jqXHR) {
                ...
                setTimeout(function () {
                    if (isEmpty(data) || isEmpty(data.error)) {
                        ...
                        //------------------------Customer Violate-----------------------
                        self.raise('fileuploaded', [outData, $thumb.attr('id'), i]);
                    } else {
        ...
        change: function (e) {
            ...
            //---------------------------Customer Violate--------------------------
            total += $preview.find('.file-preview-initial').length || 0;
            //---------------------------Customer Violate--------------------------
