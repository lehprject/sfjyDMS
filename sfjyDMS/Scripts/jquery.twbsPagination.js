...
(function ($, window, document, undefined) {
    ...
    TwbsPagination.prototype = {
        ...
        show: function (page) {
            ...
            this.render(this.getPages(page), this.options.totalPages);
            this.setupEvents();
        ...
        buildListItems: function (pages) {
            ...
            **/
            ...
            if (this.options.first) {
                arr.push(this.buildItem('first', 1));
            }

            if (this.options.prev) {
                var prev = pages.currentPage > 1 ? pages.currentPage - 1 : this.options.loop ? this.options.totalPages : 1;
                arr.push(this.buildItem('prev', prev));
            }

            if (pages.numeric[0] > 1 && this.options.intervalText) {
                arr.push(this.buildNonItem('interval'));
            }

            for (var i = 0; i < pages.numeric.length; i++) {
                arr.push(this.buildItem('page', pages.numeric[i]));
            }

            if (pages.numeric[pages.numeric.length - 1] < this.options.totalPages && this.options.intervalText) {
                arr.push(this.buildNonItem('interval'));
            }

            if (this.options.next) {
                var next = pages.currentPage < this.options.totalPages ? pages.currentPage + 1 : this.options.loop ? 1 : this.options.totalPages;
                arr.push(this.buildItem('next', next));
            }

            if (this.options.last) {
                arr.push(this.buildItem('last', this.options.totalPages));
            }

            if (this.options.totalPagesText) {
                arr.push(this.buildNonItem('totalpages', this.options.totalPages));
            }
        ...
        buildNonItem: function (type, content) {
            var itemContainer = $('<li></li>'),
            ...
            switch (type) {
                case 'totalpages':
                    itemText = this.options.totalPagesText.replace('{0}', content);
                    itemContainer.addClass(this.options.pageClass);
                    break;
                case 'interval':
                    itemText = this.options.intervalText;
                    itemContainer.addClass(this.options.pageClass);
                    break;
                default:
                    break;
        ...
        render: function (pages, totalpages) {
    ...
    $.fn.twbsPagination.defaults = {
        ...
        next: '下一页',
        last: '尾页',
        totalPagesText: '共<strong>{0}</strong>页',
        intervalText: '...',
        loop: false,
        onPageClick: null,
